...
insert (Node y left right) x
  ...
  | otherwise = Node y left right -- x == y, không thêm phần tử trùng lặp
...
treeSort :: (Ord a) => [a] -> [a]
treeSort xs = inOrder (insertList Empty xs)
  where
    inOrder :: Tree a -> [a]
    inOrder Empty = []
...
hasPath (Node x left right) a b
  | a == x = search (Node x left right) b
  | a < x = hasPath left a b
  | otherwise = hasPath right a b
  where
    search :: (Ord a) => Tree a -> a -> Bool
    search Empty _ = False
    search (Node y left right) z
      | z == y = True
      | z < y = search left z
      | otherwise = search right z
...
isValidMove piece (x1, y1) (x2, y2)
  | not (isValidPos (x1, y1)) || not (isValidPos (x2, y2)) = False
  | otherwise = case piece of
      King   -> abs (x2 - x1) <= 1 && abs (y2 - y1) <= 1
      Rook   -> x1 == x2 || y1 == y2
      Knight -> (abs (x2 - x1) == 2 && abs (y2 - y1) == 1) || (abs (x2 - x1) == 1 && abs (y2 - y1) == 2)
  where
    isValidPos (x, y) = x >= 0 && x <= 7 && y >= 0 && y <= 7

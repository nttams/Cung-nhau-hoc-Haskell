...
data Contry = France | Italy | Spain deriving (Show)
type Region = (RegionName,Contry)
regions :: [Region]
...
data Kind = Red { alcohol :: Double }| White { alcohol :: Double } | Rose { alcohol :: Double } deriving (Show)

wines :: [Kind]
wines = [Red { alcohol = 14.5 },White { alcohol = 13 },Rose { alcohol = 12 }]
...
data Label = Wine { grape :: [Grape] , region :: Region, kind :: Kind} deriving (Show)
larrosaRose = Wine ["Garnacha"] ("Rioja","Spain") Rose{alcohol=14} 
castiglioni = Wine ["Sangiovese"] ("Tuscany","Italy") Red{alcohol=12.5} 
lePetitHaitLafitte = Wine ["Cabernet-sauvignon","Merlot"] ("Bordeaux","France") Red{alcohol=13.5} 
...
containsGrape grape_list checkGrape = any (\wine  -> elem checkGrape $ grape wine) grape_list

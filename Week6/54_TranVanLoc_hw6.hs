...
factorialString 0 = "1 = 1"
factorialString n = concatMap (\x -> show x ++ "*") [1..n] ++ show (factorial n)
...
lookupPrice :: String -> [(String, Double)] -> Double
lookupPrice name prices = case lookup name prices of
  Just price -> price
  Nothing -> error ("Beer " ++ name ++ " not found in price list")

calculateOrderCost :: [(String, Double)] -> [(String, Double)] -> Double
calculateOrderCost prices order = sum [lookupPrice name prices * quantity | (name, quantity) <- order]

totalCostIncludingDelivery :: [(String, Double)] -> [(String, Double)] -> Double -> Double
totalCostIncludingDelivery prices order delivery = calculateOrderCost prices order + delivery

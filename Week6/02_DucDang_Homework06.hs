...
displayFactorial :: Int -> String
displayFactorial n
  | n <= 1 = show 1
  | n > 1    = "1*2* ... *" ++ show n ++ " = " ++ show (factorial n)
...
kindOfBeers :: [(String, Double)] -> [String]
kindOfBeers [] = []
kindOfBeers (x:xs) = fst x : kindOfBeers xs

kindOfBeerPrices :: [(String, Double)]
kindOfBeerPrices = [y | y <- bevogBeerPrices, any (\x -> fst y == x) $ kindOfBeers orderList]


calculateCost :: Double
calculateCost = deliveryCost +
                sum ([snd x * snd y | x <- orderList, y <- kindOfBeerPrices, fst x == fst y])
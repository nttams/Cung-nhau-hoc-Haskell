...
concat' :: [[a]]->[a]
-- concat' = foldr (\list acc-> foldr (\x acc->x:acc) acc list) [] 
concat' = foldl (++) []
...
factorial n
 ...
 |otherwise = foldl (\acc x->acc ++ show(x)++"*") [] [1..n-1] ++ show n ++ "=" ++ show result
    where
      result = foldl (*) 1 [1..n]
...
caculateCost :: [(String, Double)] -> Double
caculateCost orderList = foldl (\acc tuple->acc+beerMatch tuple) 0 orderList + deliveryCost
  where
    beerMatch (x,y) = foldl (\acc (a,b)-> if x==a then acc +b*y else acc) 0 bevogBeerPrices

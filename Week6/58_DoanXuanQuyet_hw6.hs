...
displayCalculation :: [Int]->String
displayCalculation [] = ""
displayCalculation (n:ns) = show n ++ "*" ++ displayCalculation ns

factorialString :: Int -> String
factorialString n
    | n < 2    = "1 = 1"
    | otherwise = displayCalculation [1..(n-1)] ++ (show n) ++ "=" ++  (show ( factorial n))

-- Example usage:
-- putStrLn $ factorialString 5  -- "1*2*3*4*5 = 120"
-- putStrLn $ factorialString 0  -- "1 = 1"
...
calculateBeerCost :: [(String, Double)] -> [(String, Double)] -> Double
calculateBeerCost [] _ = 0
calculateBeerCost ((_,number):numbers) ((_,price):prices)  = number * price + (calculateBeerCost numbers prices)

calculateCost :: [(String, Double)] -> Double
calculateCost [] = 0
calculateCost x = calculateBeerCost x bevogBeerPrices + deliveryCost

-- calculateCost orderList
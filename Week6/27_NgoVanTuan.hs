...
replicate' n x = take (fromInteger n) (repeat' x)
...
factorial n
  | n < 0 = "Khong the tinh giai thua so am"
  | n == 0 = "0! = 1"
  | otherwise = showstr 2 "1" ++ "=" ++ show ans
  where 
    showstr x string 
      | x > n = string
      | otherwise = showstr (x + 1) (string ++ "*" ++ show x)
    ans = product [1..n]

-- Question 8
-- Below you have defined some bia prices in bevogBeerPrices and your order list in
...
totalCost :: [(String, Double)] -> [(String, Double)] -> Double
totalCost [] _ = 0
totalCost ((bia, numbers):orders) prices = numbers * (lookupPrice bia prices) + totalCost orders prices
  where
    lookupPrice _ [] = 0
    lookupPrice bia ((b, price):ds)
      | bia == b  = price
      | otherwise  = lookupPrice bia ds
calculateTotalCost :: [(String, Double)] -> [(String, Double)] -> Double -> Double
calculateTotalCost orders beerPrices delivery = totalCost orders beerPrices + delivery

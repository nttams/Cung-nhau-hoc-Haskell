...
sumString = sum([read [x] | x <- show (2^1000)])
...
isValidNucleotidesString :: String -> Bool
isValidNucleotidesString s = and [x `elem` "GCTA" | x <- s]

dnaToRnaChar :: Char -> Char
dnaToRnaChar c = case c of
    'G' -> 'C'
    'C' -> 'G'
    'T' -> 'A'
    'A' -> 'U'
...
toRNA :: String -> String
toRNA s
    | length s <= 0 = "Empty DNAs"
    | isValidNucleotidesString s = [dnaToRnaChar x | x <- s]
...
rockPaperScissor :: [Char] -> [Char] -> (Int, Int)
rockPaperScissor s1 s2 =
    let scores = [getScore t | t <- zip s1 s2 ]
        scoreA = sum[fst t | t <- scores]
        scoreB = sum[snd t | t <- scores]
    in (scoreA, scoreB)
...
sumDigits digits = sum [(charToNum x) | x <- show (digits)]
  where
...
toRNA dna
  ...
  | checkDNA dna == True =
    "Invalid DNA sequence. Use only nucleotides A, C, G, or T."
  | otherwise = convertDNA dna
  where
    checkDNA :: String -> Bool
    checkDNA [] = True
    checkDNA xs = False `elem` [x `elem` "ACGT" | x <- xs]
    ...
    convertChar x =
      case x of
        ...
        'T' -> 'A'
        'A' -> 'U'
    convertDNA :: String -> String
    convertDNA [] = []
    convertDNA xs = [convertChar x | x <- xs]
...
(7,4)
-}
-- rockPaperScissor :: String -> String -> (Int, Int)
rockPaperScissor :: String -> String -> (Int, Int)
rockPaperScissor [] [] = (0, 0)
rockPaperScissor xs ys =
  ( sum [fst (caculatePoint a) | a <- (zip xs ys)]
  , sum [snd (caculatePoint a) | a <- (zip xs ys)])
  where
    caculatePoint :: (Char, Char) -> (Int, Int)
    caculatePoint ('R', 'R') = (1, 1)
    caculatePoint ('R', 'P') = (0, 3)
    caculatePoint ('R', 'S') = (3, 0)
    caculatePoint ('P', 'R') = (3, 0)
    caculatePoint ('P', 'P') = (1, 1)
    caculatePoint ('P', 'S') = (0, 3)
    caculatePoint ('S', 'R') = (0, 3)
    caculatePoint ('S', 'P') = (3, 0)
    caculatePoint ('S', 'S') = (1, 1)
    caculatePoint (_, _)     = (0, 0)

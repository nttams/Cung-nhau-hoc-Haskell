...
2318
-}
-- kiểm tra nếu số đầu vào là bội của số khác
isMultipleOf :: Int -> Int -> Bool
isMultipleOf x y = x `mod` y == 0
-- tổng các bội số của hoặc 3,5,  nhỏ hơn n 
sumMultiples :: Int -> Int
sumMultiples n = sum [x | x <- [1..n-1], isMultipleOf x 3 || isMultipleOf x 5]
...
sumDigits = sum . map (subtract 48 . fromEnum) . show
...
toRNA "" = "Khong co DNAs"
toRNA dna
    | any (`notElem` "ACGT") dna = "Chuoi DNA khong hop le. Chi dung nucleotides A, C, G, or T."
    | otherwise = map transcribe dna
    where
        transcribe 'G' = 'C'
        transcribe 'C' = 'G'
        transcribe 'T' = 'A'
...
(7,4)
-}
-- Hàm tính điểm số từ kết quả một lượt chơi
calculateScore :: Char -> Char -> (Int, Int)
calculateScore 'R' 'S' = (3, 0)
calculateScore 'S' 'P' = (3, 0)
calculateScore 'P' 'R' = (3, 0)
calculateScore 'S' 'R' = (0, 3)
calculateScore 'P' 'S' = (0, 3)
calculateScore 'R' 'P' = (0, 3)
calculateScore _ _ = (1, 1) 

-- Hàm tính tổng điểm của A và B từ danh sách các nước đi của họ
rockPaperScissor :: [Char] -> [Char] -> (Int, Int)
rockPaperScissor movesA movesB = foldl (\(a, b) (x, y) -> (a + x, b + y)) (0, 0) scores
    where scores = zipWith calculateScore movesA movesB

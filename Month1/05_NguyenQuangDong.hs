...
sumDigits = sum . map (read . return) . show   -- convert number to string, then convert each character to number, then sum all

sumOfDigitsOfTwoToPowerOfThousand :: Integer
sumOfDigitsOfTwoToPowerOfThousand = sumDigits (2^1000)
...
toRNA "" = "Empty DNAs"
toRNA dna 
  | all (`elem` "ACGT") dna = map transcribe dna   -- checks if all characters in "ACGT"
  | otherwise = "Invalid DNA sequence. Use only nucleotides A, C, G, or T."
  where
    transcribe 'G' = 'C'
    transcribe 'C' = 'G'
    transcribe 'T' = 'A'
...
-}
rockPaperScissor :: [Char] -> [Char] -> (Int, Int)
rockPaperScissor as bs = (sum [fst score | score <- scores], sum [snd score | score <- scores])
  where
    scores = [score a b | (a, b) <- zip as bs]

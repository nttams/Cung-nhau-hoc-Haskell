...
listFiles = do
  ...
  let countNum = length contents
  putStrLn $ show (countNum)
...
createMsg = do
  ...
  putStrLn "Message Saved!"
  contents <- readFile "msg.txt"
  putStrLn $ "Your message : " ++ contents
...
timeIO action = do
  ...
  result <- action
  timeFinish <- getCPUTime
  let diff = fromInteger (timeFinish - timeStart) / 10 ^ 12
  putStrLn $ "timeToExecute = " ++ show diff
...
benchmark = do
  ...
  value <- read <$> getLine
  let listPrime =
        [(primes1, "primes1"), (primes2, "primes2"), (primes3, "primes3")]
      resultsIO = map (processItem value) listPrime
  results <- sequence resultsIO
  mapM_
    (\(name, largest, diff) ->
       putStrLn
         $ "Name of function : "
             ++ name
             ++ "\n  The largest prime result : "
             ++ largest
             ++ "\n     Time to excute : "
             ++ diff
             ++ " second"
  ...
  where
    ...
    processItem value (prime, name) = do
      start <- getCPUTime
      let resutl = prime value
      end <- getCPUTime
      let diff = show (fromInteger (end - start) / 10 ^ 12)
          largest = show . last $ resutl
      return (name, largest, diff)
...
printEntry prefix currentDir = do
  filePaths <- listDirectory currentDir
  mapM_ (printTree prefix currentDir) (zip (init filePaths) (repeat False))
  mapM_ (printTree prefix currentDir) [(last filePaths, True)]
